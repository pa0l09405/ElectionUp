...
public class ConsiglieriReal extends Activity implements OnClickListener {
	...
	public void inizializzaDBConsiglieri() {
		...
		if (htmlRisposta != null) {
			...
			try {
				...
				for (int i = 0; i < jsonA.length(); i++) {
					...
					db3.insert(object.getString("id"),
							object.getString("nome"),
							object.getString("cognome"),
							object.getString("sesso"), 0);
				}
			...
			do {
				...
				bottone.setOnClickListener(new View.OnClickListener() {
					public void onClick(View v) {
						...
						db3.update(
								v.getTag().toString(),
								db3.selectVotiConsiglieri(v.getTag().toString()) + 1);

						Intent intent = new Intent(ConsiglieriReal.this,
								ConsiglieriReal2.class);
						Bundle bundle = new Bundle();
						bundle.putString("access_token", access_token);
						bundle.putString("refresh_token", refresh_token);
						bundle.putString("sesso",
								db3.selectSessoConsiglieri(v.getTag().toString()));
						Log.i("Sesso consiglieri=>", db3.selectSessoConsiglieri(v.getTag().toString()));
